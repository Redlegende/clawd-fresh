#!/usr/bin/env python3
"""Interactive Garmin data fetcher"""
...
print("If MFA is required, you'll be prompted for the code.\n")

def mfa_callback():
    code = input("🔐 Enter MFA code from your phone/email: ").strip()
    return code
...
try:
    ...
    if "MFA" in str(e) or "token" in str(e).lower():
        print("🔐 MFA required!")
        # Re-initialize with MFA callback
        client = Garmin(EMAIL, PASSWORD)
...
# Fetch last 7 days
days = 7
print(f"📊 Fetching last {days} days of fitness data...")
...
for i in range(days):
    ...
    data.append(daily)
    print(f"  ✓ {date_str}: Sleep={daily['sleep_score']}, HR={daily['resting_hr']}, VO2={daily['vo2_max']}, Steps={daily['steps']}")
...
with open(sql_file, 'w') as f:
    f.write("-- Garmin data import for Supabase\n")
    f.write("INSERT INTO fitness_metrics (date, user_id, body_battery, vo2_max, sleep_score, sleep_hours, resting_hr, steps) VALUES\n")
    values = []
    for d in data:
        user_id = "'b4004bf7-9b69-47e5-8032-c0f39c654a61'"  # Jakob's user ID
        vals = f"    ('{d['date']}', {user_id}, {d['body_battery'] or 'NULL'}, {d['vo2_max'] or 'NULL'}, {d['sleep_score'] or 'NULL'}, {d['sleep_hours'] or 'NULL'}, {d['resting_hr'] or 'NULL'}, {d['steps'] or 'NULL'})"
        values.append(vals)
    f.write(",\n".join(values))
...
print("\n🎉 Done! To upload to Supabase, run:")
print(f"   cat {sql_file}")
print("   Then paste into Supabase SQL Editor")