#!/usr/bin/env python3
"""Fetch 30 days of Garmin data - reads MFA from file or env"""
...
if mfa:
    ...
    try:
        ...
        print(f"🔐 MFA required! Please set GARMIN_MFA env var or create {MFA_FILE} with the code")
        print(f"   Latest Garmin MFA email ID: 19c232bd4592c786")
        sys.exit(1)
...
for i in range(days):
    ...
    data.append(daily)
    if i < 3 or i % 5 == 0:
        print(f"  ✓ {date_str}: Sleep={daily['sleep_score']}, BB={daily['body_battery']}, HR={daily['resting_hr']}, Steps={daily['steps']}")
...
print(f"\n🎉 Fetched {len(data)} days, {len(sql_lines)} days with data")

# Clean up MFA file if it exists
if os.path.exists(MFA_FILE):
    os.remove(MFA_FILE)
