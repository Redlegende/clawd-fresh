...
if [[ "$WORKPLACE" == "all" ]]; then
  FILTER="date=gte.${MONTH}-01&date=lt.${MONTH}-32&paid=eq.false"
else
  FILTER="date=gte.${MONTH}-01&date=lt.${MONTH}-32&source=eq.$WORKPLACE&paid=eq.false"
fi
...
RESPONSE=$(curl -s -X PATCH "$SUPABASE_URL/rest/v1/finance_entries?${FILTER}" \
  ...
  -d '{"paid": true, "paid_at": "'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}')
...
if [[ "$COUNT" -gt 0 ]]; then
  ...
  echo "ℹ️  No unpaid entries found for $MONTH"
fi
